...
import com.id22.movieapp.databinding.FragmentNowPlayingBinding
...
class NowPlayingFragment : BaseFragment<FragmentNowPlayingBinding>(),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (activity != null) {
            bind.srRefresh.setOnRefreshListener(this)
    ...
    override fun onRefresh() {
        bind.srRefresh.isRefreshing = false
        getAllPlayingMovie()
    }

    private fun setTitleActionBar() {
        (activity as MainActivity?)?.setToolbarVisible(
            title = getString(R.string.menu_now_playing)
        )
    }
    ...
    private fun getAllPlayingMovie() {
        movieViewModel.getNowPlaying().observe(viewLifecycleOwner) { result ->
            if (result != null) {
                when (result) {
                    is Resource.Success -> {
                        bind.tvMessage.visibility = View.GONE
