...
import com.id22.movieapp.databinding.FragmentTopRatedBinding
...
class TopRatedFragment : BaseFragment<FragmentTopRatedBinding>() {
    ...
    private fun setTitleActionBar() {
        (activity as MainActivity?)?.setToolbarVisible(
            title = getString(R.string.menu_top_rated)
        )
    }
    ...
    private fun setAdapter() {
        ...
        movieAdapter.setOnItemClickCallback(object : MovieAdapter.ActionAdapter {
            override fun onItemClick(data: Movie) {
                val args = TopRatedFragmentDirections
                    .actionToDetailMovie()
                    .setMovie(data)
                    .arguments
    ...
    private fun getAllTopRatedMovie() {
        movieViewModel.getTopRated().observe(viewLifecycleOwner) { result ->
            if (result != null) {
                when (result) {
                    is Resource.Success -> {
                        bind.tvMessage.visibility = View.GONE
