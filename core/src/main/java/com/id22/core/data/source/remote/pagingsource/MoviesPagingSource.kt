...
class MoviesPagingSource(
    ...
    private val typeMovie: String? = null
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, MovieResponse> {
        ...
        return try {
            val res = when (typeMovie) {
                "playing" -> {
                    apiService.getAllNowPlayingMovie(page)
                }
                "popular" -> {
                    apiService.getAllPopularMovie(page)
                }
                else -> {
                    apiService.getAllTopRatedMovie(page)
                }
            }
